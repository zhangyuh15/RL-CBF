...
def LINEAR(x, hidden, name="None"):
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
        ...
        weight = tf.get_variable(
            "Weight", [x.get_shape()[-1], hidden], initializer=tf.truncated_normal_initializer(stddev=0.02)
        )  # Near empty NN
...
class GetValue:
    def __init__(self, sess, variable_list, name=None):
        self.name = name
        self.sess = sess
        self.op_list = tf.concat(
            axis=0, values=[tf.reshape(v, [np.prod(v.get_shape().as_list())]) for v in variable_list]
        )
...
def DISCOUNT_SUM(x, discount_factor, print_info=None):
    ...
    for idx, value in enumerate(x[::-1]):
        # if value.shape
        discount_sum[: size - idx] += np.squeeze(value)
        if size - idx - 1 == 0:
            break
