...
class TRPO:
    ...
    def build_model(self):
        ...
        kl = (
            GAUSS_KL(self.old_action_dist_mu, self.old_action_dist_logstd, self.action_dist_mu, self.action_dist_logstd)
            / batch_size
        )
    ...
    def train(self, iteration):
        ...
        action_dist_mu = np.squeeze(np.concatenate([each_path["Action_RL_mu"] for each_path in batch_path]))
        ...
        action = np.squeeze(np.concatenate([each_path["Action_RL"] for each_path in batch_path]))
        ...
        feed_dict = {
            ...
            self.old_action_dist_logstd: np.expand_dims(action_dist_logstd, axis=1),
        }
    ...
    def build_policy(self, states, name="Policy"):
        ...
        with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
            ...
            action_dist_logstd = tf.get_variable(
                "logstd", initializer=tf.constant_initializer(0), shape=[1, self.action_size]
            )
    ...
    def act(self, obs):
        ...
        action_dist_mu, action_dist_logstd = self.sess.run(
            [self.action_dist_mu, self.action_dist_logstd], feed_dict={self.obs: obs_expanded}
        )
    ...
    def rollout(self):
        ...
        while timesteps < self.args.timesteps_per_batch:
            ...
            for i in range(self.args.max_path_length):
                ...
                # Get GP dynamics
                if self.firstIter == 1:
                    [f, g, x, std] = dynamics_gp.get_GP_dynamics(self, prev_obs_expanded, action_RL, t)
                else:
                    [f, g, x, std] = dynamics_gp.get_GP_dynamics_prev(self, prev_obs_expanded, action_RL, t)
