...
# -*- coding: utf-8 -*-
"""
Created on Sat Aug 18 16:34:37 2018
...
def build_barrier(self):
    ...
    self.P = matrix(np.diag([1.0, 1e18]), tc="d")
...
def control_barrier(self, obs, u_rl, f, g, x, std):
    ...
    h = np.array(
        [
            ...
            - kd * np.dot(np.abs(self.H1), std),
            gamma_b * self.F
            + np.dot(self.H2, f)
            + np.dot(self.H2, g) * u_rl
            - (1 - gamma_b) * np.dot(self.H2, x)
            - kd * np.dot(np.abs(self.H2), std),
            gamma_b * self.F
            + np.dot(self.H3, f)
            + np.dot(self.H3, g) * u_rl
            - (1 - gamma_b) * np.dot(self.H3, x)
            - kd * np.dot(np.abs(self.H3), std),
            gamma_b * self.F
            + np.dot(self.H4, f)
            + np.dot(self.H4, g) * u_rl
            - (1 - gamma_b) * np.dot(self.H4, x)
            - kd * np.dot(np.abs(self.H4), std),
            gamma_b * self.F
            + np.dot(self.H5, f)
            + np.dot(self.H5, g) * u_rl
            - (1 - gamma_b) * np.dot(self.H5, x)
            - kd * np.dot(np.abs(self.H5), std),
            gamma_b * self.F
            + np.dot(self.H6, f)
            + np.dot(self.H6, g) * u_rl
            - (1 - gamma_b) * np.dot(self.H6, x)
            - kd * np.dot(np.abs(self.H6), std),
            gamma_b * self.F
            + np.dot(self.H7, f)
            + np.dot(self.H7, g) * u_rl
            - (1 - gamma_b) * np.dot(self.H7, x)
            - kd * np.dot(np.abs(self.H7), std),
            gamma_b * self.F
            + np.dot(self.H8, f)
            + np.dot(self.H8, g) * u_rl
            - (1 - gamma_b) * np.dot(self.H8, x)
            - kd * np.dot(np.abs(self.H8), std),
