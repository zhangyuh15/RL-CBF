...
def build_barrier(self):
    ...
    self.P = matrix(np.diag([1.0, 1e24]), tc="d")
...
def control_barrier(self, obs, u_rl, f, g, x, std):
    ...
    G = np.array(
        [
            [-np.dot(self.H1, g), -np.dot(self.H2, g), -np.dot(self.H3, g), -np.dot(self.H4, g), 1, -1, g[1], -g[1]],
            [-1, -1, -1, -1, 0, 0, 0, 0],
        ]
    )
    G = np.transpose(G)
