...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--summary-dir", help="directory for storing tensorboard info", default="./results/pendulum_ddpg"
    )
    ...
    args["summary_dir"] = os.path.join(
        base_dir, "results", "pendulum_ddpg", "exp-" + datetime.datetime.now().strftime("%y-%m-%d-%H-%M")
    )
    ...
    savemat(
        os.path.join(args["summary_dir"], "data4_" + datetime.datetime.now().strftime("%y-%m-%d-%H-%M") + ".mat"),
