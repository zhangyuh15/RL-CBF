...
class LEARNER:
    def __init__(self, args, sess, simulator):
        ...
        self.simulator.unwrapped.max_torque = 15.0
        self.simulator.unwrapped.max_speed = 60.0
        self.simulator.unwrapped.action_space = spaces.Box(
            low=-self.simulator.unwrapped.max_torque, high=self.simulator.unwrapped.max_torque, shape=(1,)
        )
        high = np.array([1.0, 1.0, self.simulator.unwrapped.max_speed])
    ...
    def learn(self):
        ...
        while True:
            ...
            if total_steps > self.args.total_train_step:
                savemat(
                    "data4_" + datetime.datetime.now().strftime("%y-%m-%d-%H-%M") + ".mat",
